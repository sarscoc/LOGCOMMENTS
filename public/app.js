...
const state = { roomId: null, room: null, annotations: [], presence: [], parsed: null, selection: null, pendingSelection: null, replyTo: null, editingCommentId: null, editingOriginalPersona: null, commentImage: "", activeTabIndex: 0, carouselPosition: 1, isSliding: false, slideQueue: 0, readingAnchor: "", syncingScrollUntil: 0, viewMode: localStorage.getItem("trpgMarkerViewMode") || "compact", mainTab: "", hiddenTabs: new Set(), timelineSlots: [], glimpseFrame: 0, glimpseTimer: 0, suggestionTimer: null, newPersonaIcon: "", profile: null, legacyPersonas: [], lastPersona: "PL", poller: null, presencePoller: null, isTyping: false, typingTimer: null };
const uid = () => crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2) + Date.now();
const esc = value => String(value ?? "").replace(/[&<>\"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
function commentBodyHtml(value){const urlPattern=/(https:\/\/[^\s<>"']+)/g,isImage=url=>/\.(?:png|jpe?g|gif|webp|avif)(?:[?#]|$)/i.test(url)||/[?&]format=(?:png|jpe?g|gif|webp|avif)(?:&|$)/i.test(url)||/(?:i\.imgur\.com|cdn\.discordapp\.com|media\.discordapp\.net|pbs\.twimg\.com|files\.catbox\.moe|oaiusercontent\.com)/i.test(url);return String(value||"").split(urlPattern).map(part=>{if(!/^https:\/\//.test(part))return esc(part);const safe=esc(part);return isImage(part)?`<img class="comment-image url-image" data-expand-image src="${safe}" alt="引用画像" loading="lazy" referrerpolicy="no-referrer">`:`<a class="comment-link" href="${safe}" target="_blank" rel="noopener noreferrer">${safe}</a>`}).join("")}
state.archiveMode=false;state.deleteTarget=null;
const safeFilename=value=>(String(value||"TRPGログ").replace(/[\\/:*?"<>|]/g,"_").trim()||"TRPGログ").slice(0,80);
function archiveData(room,annotations,personas=[]){return{format:"trpg-log-marker",version:1,exportedAt:new Date().toISOString(),room:{title:room.title||"TRPG LOG",createdAt:room.createdAt||"",tabs:room.tabs||[],messages:room.messages||[]},annotations:(annotations||[]).map(({liked_by_me,...item})=>item),personas}}
...
function downloadArchive(room,annotations,personas=[]){const data=archiveData(room,annotations,personas),json=JSON.stringify(data),blob=makeZip([{name:"room.trpglog",data:json},{name:"index.html",data:standaloneArchiveHtml(data)}]),link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download=`${safeFilename(room.title)}.zip`;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1500)}
...
async function openArchiveFile(file){try{const data=await readArchiveFile(file);if(data?.format!=="trpg-log-marker"||!data.room?.messages?.length)throw new Error("TRPG LOG MARKERの保存ファイルではありません");clearInterval(state.poller);clearInterval(state.presencePoller);state.archiveMode=true;state.roomId="archive";state.room={...data.room,id:"archive"};state.annotations=data.annotations||[];state.presence=[];state.profile.personas=data.personas||state.profile.personas;state.hiddenTabs=new Set();state.activeTabIndex=0;state.carouselPosition=1;state.mainTab=state.room.tabs[0]||"";$("#homeView").classList.add("hidden");$("#roomView").classList.remove("hidden");$("#profileBtn").classList.add("hidden");$("#shareBtn").classList.add("hidden");$("#roomTitle").textContent=state.room.title;$("#roomStatus").textContent="保存ZIPを表示中（クラウド容量は使いません）";$("#tabFilter").innerHTML='<option value="">メインタブを選択</option>';state.room.tabs.forEach(tab=>$("#tabFilter").insertAdjacentHTML("beforeend",`<option>${esc(tab)}</option>`));$("#tabFilter").value=state.mainTab;renderComments();renderLog();document.title=`${state.room.title} | 保存版`}catch(error){$("#homeStatus").textContent=error.message}}
...
async function confirmDeleteRoom(saveFirst){const target=state.deleteTarget;if(!target)return;const saveButton=$("#saveDeleteRoomBtn"),deleteButton=$("#deleteOnlyRoomBtn");saveButton.disabled=deleteButton.disabled=true;try{if(saveFirst){const data=await fetchArchiveForRoom(target.id);downloadArchive(data.room,data.annotations,data.personas)}await removeOwnedRoom(target.id);$("#deleteRoomDialog").close();state.deleteTarget=null}catch(error){alert(error.message)}finally{saveButton.disabled=deleteButton.disabled=false}}
...
$("#exportRoomBtn").onclick=()=>state.room&&downloadArchive(state.room,state.annotations,state.profile.personas);
...
const startupUrl=new URL(location.href),siteOwnerKey=startupUrl.searchParams.get("owner");if(siteOwnerKey){localStorage.setItem("trpgMarkerSiteOwnerKey",siteOwnerKey);startupUrl.searchParams.delete("owner");history.replaceState(null,"",startupUrl.pathname+startupUrl.search)}const roomId=startupUrl.searchParams.get("room"); if(roomId)openRoom(roomId);else{renderOwnedRooms();renderRecentRooms()}
