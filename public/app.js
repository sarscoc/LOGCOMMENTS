...
function renderComments() {
  ...
  const roots=annotations.filter(annotation=>!annotation.parent_id||!state.annotations.some(candidate=>candidate.id===annotation.parent_id));$("#commentsList").innerHTML=roots.length?roots.map(annotation=>cardHtml(annotation)).join(""):'<p class="empty">マーカーされた感想がここに並びます。</p>';
}
