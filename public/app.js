...
function showMainSuggestions(time){hideTabSuggestions();if(activeTabName()!==state.mainTab)return;const visibleTimes=new Set(visibleSuggestionTimes()),target=minuteValue(time);let suggestions=state.room.messages.filter(message=>message.tab!==state.mainTab&&message.text.trim()&&visibleTimes.has(message.time));if(!suggestions.length&&time)suggestions=state.room.messages.filter(message=>message.tab!==state.mainTab&&message.text.trim()&&message.time===time);if(!suggestions.length&&target!=null)suggestions=state.room.messages.filter(message=>{const value=minuteValue(message.time);return message.tab!==state.mainTab&&message.text.trim()&&value!=null&&Math.abs(value-target)<=2});suggestions=suggestions.slice(0,5);if(!suggestions.length)return;$("#tabSuggestions").innerHTML=`<small>OTHER TABS${time?` · ${esc(time)}`:""}</small>${suggestions.map(message=>`<button data-suggest-message="${esc(message.id)}"><b>${esc(message.tab)}</b><span>${esc(message.speaker||"")}${message.speaker?"：":""}${esc(message.text.slice(0,90))}</span></button>`).join("")}`;$("#tabSuggestions").classList.remove("hidden");state.suggestionTimer=setTimeout(hideTabSuggestions,7000)}
...
function switchLogPage(direction) {
  ...
  if(activeTabName()===state.mainTab)setTimeout(()=>showMainSuggestions(time),80);else hideTabSuggestions();
}
