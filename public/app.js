...
const state = { roomId: null, room: null, annotations: [], presence: [], parsed: null, selection: null, pendingSelection: null, replyTo: null, editingCommentId: null, commentImage: "", activeTabIndex: 0, carouselPosition: 1, isSliding: false, slideQueue: 0, readingAnchor: "", syncingScrollUntil: 0, viewMode: localStorage.getItem("trpgMarkerViewMode") || "compact", mainTab: "", hiddenTabs: new Set(), timelineSlots: [], glimpseFrame: 0, glimpseTimer: 0, suggestionTimer: null, newPersonaIcon: "", profile: null, legacyPersonas: [], lastPersona: "PL", poller: null, presencePoller: null, isTyping: false, typingTimer: null };
...
function renderVisibleGlimpses(){cancelAnimationFrame(state.glimpseFrame);state.glimpseFrame=requestAnimationFrame(()=>{document.querySelectorAll(".foreign-glimpses").forEach(el=>el.remove());if(state.viewMode!=="timeline")return;const panel=document.querySelector(`.log-page[data-track-index="${state.carouselPosition}"]`),scroll=panel?.querySelector(".page-scroll");if(!scroll)return;const bounds=scroll.getBoundingClientRect(),tab=activeTabName();panel.querySelectorAll(".timeline-slot.empty-slot[data-slot-index]").forEach(row=>{const rect=row.getBoundingClientRect();if(rect.bottom<bounds.top-120||rect.top>bounds.bottom+120)return;const slot=state.timelineSlots[Number(row.dataset.slotIndex)],html=slot&&glimpseHtml(slot,tab);if(html)row.querySelector(".timeline-slot-content")?.insertAdjacentHTML("beforeend",`<div class="foreign-glimpses">${html}</div>`)})})}
...
function renderLog(anchorTime = "") {
  ...
  const panels = [];
  panels.push(pagePanelHtml(entries.at(-1).tab,entries.at(-1).index,0,grouped,search,"last"));
  entries.forEach((item,position)=>panels.push(pagePanelHtml(item.tab,item.index,position+1,grouped,search)));
  panels.push(pagePanelHtml(entries[0].tab,entries[0].index,entries.length+1,grouped,search,"first"));
...
function renderTimelineLog(anchorTime="") {
  ...
  const panels=[timelinePagePanelHtml(entries.at(-1).tab,entries.at(-1).index,0,slots,grouped,search,"last")];entries.forEach((item,position)=>panels.push(timelinePagePanelHtml(item.tab,item.index,position+1,slots,grouped,search)));panels.push(timelinePagePanelHtml(entries[0].tab,entries[0].index,entries.length+1,slots,grouped,search,"first"));
...
function renderComments() {
  ...
  const cardHtml=(a,depth=0)=>{const tab=state.room.messages.find(m=>m.id===a.message_id)?.tab||"",replies=children.get(a.id)||[],mine=a.author_id===state.profile.id;return `<div class="comment-thread ${depth?"is-reply":""}" style="--reply-depth:${Math.min(depth,3)}"><div class="comment-card" style="--comment-marker:${esc(markerColor(a.color))}" id="comment-${a.id}" data-target="${a.message_id}"><div class="comment-author">${a.persona_icon ? `<img class="comment-avatar" src="${esc(a.persona_icon)}" alt="">` : '<span class="comment-avatar empty-avatar"></span>'}<span>${esc(a.persona_name)}<span class="persona-type">${esc(a.persona_type)}</span></span><button class="comment-reply" type="button" data-reply-comment="${esc(a.id)}" title="返信">↩</button>${mine?`<button class="comment-edit" type="button" data-edit-comment="${esc(a.id)}">編集</button>`:""}<time class="comment-date">${tab?`${esc(tab)} · `:""}${esc(formatCommentDate(a.created_at))}</time></div>${a.body?`<p class="comment-body">${esc(a.body)}</p>`:""}${a.has_image?`<img class="comment-image" data-expand-image src="/api/rooms/${encodeURIComponent(state.roomId)}/annotations/${encodeURIComponent(a.id)}/image" alt="添付画像" loading="lazy">`:""}<button class="comment-like ${a.liked_by_me?"liked":""}" type="button" data-like-comment="${esc(a.id)}" aria-label="好き">♡${Number(a.like_count)||""}</button></div>${replies.map(reply=>cardHtml(reply,depth+1)).join("")}</div>`};
  const roots=annotations.filter(annotation=>!annotation.parent_id||!state.annotations.some(candidate=>candidate.id===annotation.parent_id));const typing=state.presence.filter(person=>person.is_typing&&person.typing_message_id).map(person=>({typing:true,message_id:person.typing_message_id,person}));const items=[...roots.map(annotation=>({annotation,message_id:annotation.message_id})),...typing].sort((a,b)=>(order.get(a.message_id)??Infinity)-(order.get(b.message_id)??Infinity));const typingHtml=item=>`<button type="button" class="typing-comment" data-typing-target="${esc(item.message_id)}" title="入力中のログへ移動"><span class="comment-avatar">${item.person.typing_icon?`<img src="${esc(item.person.typing_icon)}" alt="">`:esc((item.person.typing_name||item.person.pl_name||"?").slice(0,1))}</span><b>${esc(item.person.typing_name||item.person.pl_name)}</b><em>入力中…</em><i></i><i></i><i></i></button>`;$("#commentsList").innerHTML=items.length?items.map(item=>item.typing?typingHtml(item):cardHtml(item.annotation)).join(""):'<p class="empty">マーカーされた感想がここに並びます。</p>';
}
...
async function toggleLike(id){try{await api(`/api/rooms/${encodeURIComponent(state.roomId)}/annotations/${encodeURIComponent(id)}/like`,{method:"POST",body:JSON.stringify({authorId:state.profile.id})});await refreshAnnotations()}catch(error){alert(error.message)}}
...
function openCommentDialog() {
  state.replyTo=null;state.editingCommentId=null;$("#commentDeleteBtn").classList.add("hidden");
...
function clearCommentImage(){state.commentImage="";$("#commentImageInput").value="";$("#commentImagePreview").innerHTML="";$("#commentImagePreview").classList.add("hidden");$("#commentImageRemove").classList.add("hidden")}
...
document.addEventListener("click",e=>{const button=e.target.closest("[data-delete-room]");if(button)deleteOwnedRoom(button.dataset.deleteRoom,button.dataset.roomTitle)});
...
$("#commentImageInput").onchange=async e=>{const file=e.target.files[0];if(!file)return;$("#commentImagePreview").classList.remove("hidden");$("#commentImagePreview").textContent="処理中…";try{state.commentImage=await resizeCommentImage(file);$("#commentImagePreview").innerHTML=`<img src="${esc(state.commentImage)}" alt="添付予定">`;$("#commentImageRemove").classList.remove("hidden");positionCommentDialog()}catch(error){clearCommentImage();alert("画像を読み込めませんでした")}};
$("#commentImageRemove").onclick=clearCommentImage;
...
const roomId=new URLSearchParams(location.search).get("room"); if(roomId)openRoom(roomId);else{renderOwnedRooms();renderRecentRooms()}
